...
import { addWord, getWordDefinitions, getDefinitionsSample } from "@services/api/words";
...
const anotherWordStub: Definition = {
...
const wordsStub: Array<Definition> = [
    wordStub,
    anotherWordStub
];
...
describe("getWordsSample", (): void => {
    ...
    it.skip("should get word sample", async (): Promise<void> => {
        getDatabaseMock.mockResolvedValue({
            collection: (): Collection<WordDocument> => ({
                aggregate: (): AggregationCursor<Definition> => {
                    const aggregationCursor: AggregationCursor<Definition> = {
                        map: (): AggregationCursor<Definition> => (
                            aggregationCursor
                        ),
                        toArray: jest.fn()
                            .mockResolvedValueOnce(wordsStub)
                            .mockResolvedValue(["0", "1", "2", "3", "4", "5", "6"])
                    } as Partial<AggregationCursor<Definition>> as AggregationCursor<Definition>;
...
describe("getWord", (): void => {
    ...
    it("should not find word", async (): Promise<void> => {
        ...
        const wordCollection: Array<Definition> = await getWordDefinitions(slug, ip);
